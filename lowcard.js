  ...
  async handleEvent(ws, data) {
    ...
    if (!room || !this.games.has(room)) {
      if (evt === 'gameLowCardStart') {
        ...
        await this.chatServer.broadcastToRoom(room, ['gameLowCardStarted', room]);
      }
      return;
    }
    ...
    switch(evt) {
      case 'gameLowCardJoin':
        if (game.active && ws.idtarget) {
          game.players.set(ws.idtarget, { score: 0, cards: [] });
          await this.chatServer.broadcastToRoom(room, ['gameLowCardPlayerJoined', room, ws.idtarget]);
        }
        break;
        
      case 'gameLowCardNumber':
        const number = data[2];
        if (game.active && number >= 1 && number <= 6) {
          game.currentNumber = number;
          await this.chatServer.broadcastToRoom(room, ['gameLowCardNumberUpdated', room, number]);
        }
        break;
      ...
      case 'gameLowCardEnd':
        ...
        await this.chatServer.broadcastToRoom(room, ['gameLowCardEnded', room]);
        break;
    }
  }
  ...
  check() {
    ...
    try {
      ...
      if (now - this.lastMemoryLog > 60000) {
        console.log(`[MEMORY] Cloudflare Workers - memory monitoring active`);
...
export default {
  async fetch(request, env) {
    try {
      ...
      if (!env.CHAT_SERVER_2) {
        console.error("CHAT_SERVER_2 binding not found");
        return new Response("Configuration error: Missing Durable Object binding", { status: 500 });
      }
