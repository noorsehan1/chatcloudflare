...
const CONSTANTS = {
  ...
  LOCK_ACQUIRE_TIMEOUT: 3000,import { LowCardGameManager } from "./lowcard.js";
...
const CONSTANTS = {
  ...
  // New constants for better stability
...
export class ChatServer2 {
  constructor(state, env) {
    ...
    try {
      ...
      this.ipConnections = new Map(); // New: IP tracking
      this.storageUnavailable = false; // New: storage fallback mode
      ...
      this.startAggressiveMemoryCleanup(); // New
      this.startCircuitBreakerReset(); // New
      this.startIPCleanup(); // New
      this.startQueueHealthMonitor(); // New
  ...
  // New: Enforce hard limits on all maps
  enforceMapLimits() {
    ...
    if (this.updateKursiBuffer.size > limits.updateKursiBuffer) {
      const toDelete = Array.from(this.updateKursiBuffer.keys())
        .slice(0, this.updateKursiBuffer.size - limits.updateKursiBuffer);
  ...
  // New: Circuit breaker auto-reset
  ...
  // New: IP connection tracking cleanup
  startIPCleanup() {
    ...
    this._queueHealthMonitor = setInterval(() => {
      try {
        ...
        if (queueSize > CONSTANTS.QUEUE_CRITICAL_SIZE) {
          console.error(`CRITICAL: Queue size ${queueSize}, clearing old items`);
          // Force clear old queue items
          while (this.cleanupQueue?.size() > 200) {
            // Items will timeout naturally
          }
        }
  ...
  // New: Check if can accept new connection
  ...
  startMemoryMonitor() {
    this._memoryMonitor = setInterval(() => {
      try {
        ...
        // Auto-disable safe mode if memory recovers
  ...
  async getHealthStatus() {
    ...
    return {
      ...
      storageAvailable: !this.storageUnavailable
    };
  }
}
    ...
    try {
      ...
      if (url.pathname === "/metrics") {
        ...
        const metrics = {
          ...
          storageAvailable: !obj.storageUnavailable,
          timestamp: Date.now()
        };
